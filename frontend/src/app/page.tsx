          ...
          <section className="flex-1 flex flex-col h-full mx-auto w-full border-r border-border/50 lg:max-w-4xl relative">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar relative z-10">
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.role === 'ai' ? 'justify-start' : 'justify-end'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
            ...
            <div className="p-6 bg-background/80 backdrop-blur-xl border-t border-border/50 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
               <div className="relative flex gap-3 max-w-3xl mx-auto">
                 <input 
                   ...
                   className="flex-1 pl-6 pr-14 py-4 bg-card rounded-2xl focus:outline-none focus:ring-2 focus:ring-primary/50 border border-border text-foreground shadow-inner transition-all placeholder:text-muted-foreground" 
                 />
                 <button 
                   onClick={handleSearch}
                   disabled={loading || !query.trim()}
                   className="absolute right-2 top-2 bottom-2 aspect-square flex items-center justify-center bg-primary text-primary-foreground rounded-xl hover:scale-105 transition-all disabled:opacity-50 disabled:scale-100 shadow-lg shadow-primary/30"
                 >
                   <Send className="w-5 h-5 ml-0.5" />
                 </button>
               </div>
               <p className="text-center text-xs text-muted-foreground mt-3">A IA pode cometer erros. Verifique informações importantes.</p>
            </div>
          </section>
                ...
                <div className="space-y-3">
                  {loadingProposicoes ? (
                    ...
                    proposicoes.map((prop) => (
                      <div key={prop.id} className="bg-card hover:bg-accent border border-border rounded-xl p-4 shadow-sm hover:shadow-md transition-all group flex flex-col gap-2">
              ...
              <div className="space-y-4 pb-6">
                ...
                <div className="space-y-3">
                  {loadingNews ? (
                    ...
                    news.map((item, idx) => (
                      <a key={idx} href={item.url} target="_blank" rel="noopener noreferrer" className="block bg-card hover:bg-accent border border-border rounded-xl p-3 shadow-sm hover:shadow-md transition-all group">
